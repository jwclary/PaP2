...
namespace JasonClary_TimeTrackerApp
{
    class Validation
    {
        ...
        public static double ValidateDouble(string s, double min, double max)
        {
            Console.WriteLine(s);
            string response = Console.ReadLine();
            double i;

            while (!double.TryParse(response, out i) || i < min || i > max)
            {
                Console.WriteLine("Please enter a valid double within specified range");
                response = Console.ReadLine();
            }
            return i;
        }
